...
def map():
    ...
    # Default location if no coordinates are provided
    map_location = [10.8505, 76.2711]  # Default Kerala location
    if lat and lon:
        map_location = [float(lat), float(lon)]  # Use selected location

    # Create the map
    map_object = folium.Map(location=map_location, zoom_start=12 if lat and lon else 7)
    ...
    # Render the map
    return render_template('map.html', map=map_object._repr_html_(), lat=lat, lon=lon)
